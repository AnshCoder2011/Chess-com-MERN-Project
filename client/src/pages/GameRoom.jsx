...
const GameRoom = () => {
  ...
  // ✅ new states for game over modal
  ...
  useEffect(() => {
    ...
    socket.on("roomJoined", ({ color, opponent }) => {
      ...
      setTurn("w");
      if (opponent) {
        setOpponentUsername(opponent.username || (opponent?.username ?? ""));
      }
  ...
  const onDrop = (sourceSquare, targetSquare) => {
    ...
    try {
      ...
      if (chess.isGameOver()) {
        ...
        setShowWinModal(true);
        socket.emit("gameOver", { roomCode, winner });
      }
  ...
  // ✅ Win Modal
  const WinModal = () => {
    // Determine winner color (e.g., "White" or "Black")
    const winnerColor =
      winner === "Draw"
        ? "Draw"
